...
namespace AutoUpdateClient.CS
{
    public static class Manager
    {
        ...
        /// <summary>
        /// 转化成DateTime，如果不成功则返回 new DateTime()
        /// </summary>
        /// <param name="str"></param>
        /// <returns></returns>
    ...
    public static string MyUrlDeCode(string str, Encoding encoding)
    {
     ...
     return HttpUtility.UrlDecode(str, encoding); 
        ...
        public static void WriteEntry_PDD(string str)
        {
            ...
            string logFile = logDir + "\\拼多多客户问题.log";
        ...
        /// <param name="filter">显示的文件格式 Execl files (*.xls)|*.xls</param>
        /// <returns></returns>
        public static string OpenFileDialog2(string filter)
        {
            string filePath = "";
            SaveFileDialog saveFileDialog = new SaveFileDialog();
            //txt files(*.txt)|*.txt|All files(*.*)|*.*
        ...
        public static string OpenFolderDialog(string SelectedPath)
        {
            ...
            {
                folderBrowserDialog1.SelectedPath = new XMLHelpers().GetValue("OpenFolderDialog_SelectedPath");
            }
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                if (folderBrowserDialog1.SelectedPath.Trim() != "")
                {
                    ...
                    new XMLHelpers().SetValue("OpenFolderDialog_SelectedPath", path);
                }
            }
            return path;
        }
        ...
        public static List<PictureBox> GetPictureBoxs(this Control.ControlCollection cc)
        {
            ...
            foreach (PictureBox control in cc)
            {
...
        }
        ...
        public static void EmailSend(string email, string title, string content)
        {
            try
            {
                ...
                //如果需要认证，则用下面的方式
                smtp.Credentials = new NetworkCredential(new XMLHelpers().GetValue("Email_Account"), new XMLHelpers().GetValue("Email_Pwd"));
                MailMessage mm = new MailMessage(); //实例化一个邮件类
                mm.Priority = MailPriority.High; //邮件的优先级，分为 Low, Normal, High，通常用 Normal即可
                mm.From = new MailAddress(new XMLHelpers().GetValue("Email_Account"), title, Encoding.GetEncoding(936));
                //收件方看到的邮件来源；
                //第一个参数是发信人邮件地址
                //第二参数是发信人显示的名称
