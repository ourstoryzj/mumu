...
namespace DAL
{
    ...
    public class yh_corekeysService : Iyh_corekeysService
    {
        ...
        public static SqlParameter[] GetSqlParameter(yh_corekeys yh_corekeysExample)
        {
            List<SqlParameter> list_param = new List<SqlParameter>();

            if (!string.IsNullOrEmpty(yh_corekeysExample.ckname))
            {
                list_param.Add(new SqlParameter("@ckname", yh_corekeysExample.ckname));
            }
            else
            {
                list_param.Add(new SqlParameter("@ckname", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_corekeysExample.ckremark))
            {
                list_param.Add(new SqlParameter("@ckremark", yh_corekeysExample.ckremark));
            }
            else
            {
                list_param.Add(new SqlParameter("@ckremark", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_corekeysExample.cksort))
            {
                list_param.Add(new SqlParameter("@cksort", yh_corekeysExample.cksort));
            }
            else
            {
                list_param.Add(new SqlParameter("@cksort", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_corekeysExample.ckstate))
            {
                list_param.Add(new SqlParameter("@ckstate", yh_corekeysExample.ckstate));
            }
            else
            {
                list_param.Add(new SqlParameter("@ckstate", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_corekeysExample.gtid))
            {
                list_param.Add(new SqlParameter("@gtid", yh_corekeysExample.gtid));
            }
            else
            {
        ...
        public static yh_corekeys GetByReader(SqlDataReader Reader)
        {
