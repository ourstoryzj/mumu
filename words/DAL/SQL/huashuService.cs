...
namespace DAL
{
    ...
    public class huashuService
    {
        ...
        {
            DBHelper.sqlstr = "insert into  huashu (hfid,htitle,hcontext,hsort,hcount,hdate,hstate)values(" + huashuExample.hfid.ToString() + ",@htitle,@hcontext,@hsort," + huashuExample.hcount.ToString() + ",'" + huashuExample.hdate.ToString() + "',@hstate)";
        ...
        {
            DBHelper.sqlstr = "update huashu set hfid=" + huashuExample.hfid.ToString() + ",htitle=@htitle,hcontext=@hcontext,hsort=@hsort,hcount=" + huashuExample.hcount.ToString() + ",hdate='" + huashuExample.hdate.ToString("yyyy-MM-dd") + "',hstate=@hstate where hid=" + huashuExample.hid;
            //DBHelper.sqlstr = "update huashu set hfid=" + huashuExample.hfid.ToString() + ",htitle=@htitle,hcontext=@hcontext,hsort=@hsort,hcount=" + huashuExample.hcount.ToString() + ",hdate=@hdate,hstate=@hstate where hid=" + huashuExample.hid;
            return DBHelper.ExecuteNonQuery(GetSqlParameters(huashuExample));
        }
        #endregion
        ...
        public int SearchNum(string key, string state, string _type, DateTime date1, DateTime date2)
        {
            ...
            string sql2 = string.IsNullOrEmpty(key) ? " 1=1 " : " (  htitle like '%" + key + "%' or  hcontext like '%" + key + "%' ) ";
