...
namespace DAL
{
    ...
    public class basic_order_recordService : Ibasic_order_recordService
    {
        ...
        {
            string sql1 = "select  top " + (e - s + 1).ToString() + " *   from basic_order_record where ";
            ...
            DBHelper.sqlstr = sql1 + sql2 + sql3 + sql4 + sql5 + sql6 + sql11 + sql7 + sql8 + sql10 + sql9;
            List<basic_order_record> list = new List<basic_order_record>();
            SqlDataReader reader = DBHelper.ExecuteReader();
            while (reader.Read())
            {
        ...
        public static SqlParameter[] GetSqlParameters(basic_order_record basic_order_recordExample)
        {
            List<SqlParameter> list_param = new List<SqlParameter>();
            if (basic_order_recordExample.bgid != 0)
            {
                list_param.Add(new SqlParameter("@bgid", basic_order_recordExample.bgid));
            }
            else
            {
            ...
            {
                list_param.Add(new SqlParameter("@bgname", basic_order_recordExample.bgname));
            }
            else
            {
            ...
            {
                list_param.Add(new SqlParameter("@dpid", basic_order_recordExample.dpid));
            }
            else
            {
            ...
            {
                list_param.Add(new SqlParameter("@dpname", basic_order_recordExample.dpname));
            }
            else
            {
            ...
            {
                list_param.Add(new SqlParameter("@gkid", basic_order_recordExample.gkid));
            }
            else
            {
                list_param.Add(new SqlParameter("@gkid", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.gkname))
            {
                list_param.Add(new SqlParameter("@gkname", basic_order_recordExample.gkname));
            }
            else
            {
                list_param.Add(new SqlParameter("@gkname", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.ormaijiaxiu))
            {
                list_param.Add(new SqlParameter("@ormaijiaxiu", basic_order_recordExample.ormaijiaxiu));
            }
            else
            {
                list_param.Add(new SqlParameter("@ormaijiaxiu", DBNull.Value));
            }
            if (basic_order_recordExample.ordate != new DateTime() && basic_order_recordExample.ordate != null)
            {
                list_param.Add(new SqlParameter("@ordate", basic_order_recordExample.ordate.ToString("yyyy-MM-dd")));
            }
            else
            {
                list_param.Add(new SqlParameter("@ordate", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.orremark))
            {
                list_param.Add(new SqlParameter("@orremark", basic_order_recordExample.orremark));
            }
            else
            {
                list_param.Add(new SqlParameter("@orremark", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.orqq))
            {
                list_param.Add(new SqlParameter("@orqq", basic_order_recordExample.orqq));
            }
            else
            {
                list_param.Add(new SqlParameter("@orqq", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.orprepare1))
            {
                list_param.Add(new SqlParameter("@orprepare1", basic_order_recordExample.orprepare1));
            }
            else
            {
                list_param.Add(new SqlParameter("@orprepare1", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.orprepare2))
            {
                list_param.Add(new SqlParameter("@orprepare2", basic_order_recordExample.orprepare2));
            }
            else
            {
                list_param.Add(new SqlParameter("@orprepare2", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.orprepare3))
            {
                list_param.Add(new SqlParameter("@orprepare3", basic_order_recordExample.orprepare3));
            }
            else
            {
                list_param.Add(new SqlParameter("@orprepare3", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.orwangwang))
            {
                list_param.Add(new SqlParameter("@orwangwang", basic_order_recordExample.orwangwang));
            }
            else
            {
                list_param.Add(new SqlParameter("@orwangwang", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.orprice))
            {
                list_param.Add(new SqlParameter("@orprice", basic_order_recordExample.orprice));
            }
            else
            {
                list_param.Add(new SqlParameter("@orprice", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.orcode))
            {
                list_param.Add(new SqlParameter("@orcode", basic_order_recordExample.orcode));
            }
            else
            {
                list_param.Add(new SqlParameter("@orcode", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_order_recordExample.bgurl))
            {
                list_param.Add(new SqlParameter("@bgurl", basic_order_recordExample.bgurl));
            }
            else
            {
            ...
            {
                list_param.Add(new SqlParameter("@bgwuliu", basic_order_recordExample.bgwuliu));
            }
            else
            {
                list_param.Add(new SqlParameter("@bgwuliu", DBNull.Value));
            }
        ...
        public static basic_order_record GetByReader(SqlDataReader Reader)
        {
