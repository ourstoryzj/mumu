...
namespace DAL
{
    ...
    public class yh_zaociService : Iyh_zaociService
    {
        ...
        public static SqlParameter[] GetSqlParameters(yh_zaoci yh_zaociExample)
        {
            List<SqlParameter> list_param = new List<SqlParameter>();

            if (!string.IsNullOrEmpty(yh_zaociExample.zname))
            {
                list_param.Add(new SqlParameter("@zname", yh_zaociExample.zname));
            }
            else
            {
                list_param.Add(new SqlParameter("@zname", DBNull.Value));
            }
            if (yh_zaociExample.zdate != new DateTime() && yh_zaociExample.zdate != null)
            {
                list_param.Add(new SqlParameter("@zdate", yh_zaociExample.zdate));
            }
            else
            {
                list_param.Add(new SqlParameter("@zdate", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_zaociExample.zstate))
            {
                list_param.Add(new SqlParameter("@zstate", yh_zaociExample.zstate));
            }
            else
            {
                list_param.Add(new SqlParameter("@zstate", DBNull.Value));
            }
            //if (yh_zaociExample.gtid != 0)
            //{
            list_param.Add(new SqlParameter("@gtid", yh_zaociExample.gtid));
            //}
            //else
            //{
            ...
            {
                list_param.Add(new SqlParameter("@gtname", yh_zaociExample.gtname));
            }
            else
            {
                list_param.Add(new SqlParameter("@gtname", DBNull.Value));
            }
            if (yh_zaociExample.zsort != 0)
            {
                list_param.Add(new SqlParameter("@zsort", yh_zaociExample.zsort));
            }
            else
            {
                list_param.Add(new SqlParameter("@zsort", DBNull.Value));
            }
        ...
        public static yh_zaoci GetByReader(SqlDataReader Reader)
        {
