...
namespace Operation
{
    ...
    public class SystemHotkey : System.ComponentModel.Component, IDisposable
    {
        ...
        {	//Handle WM_Hotkey event
            if ((m.Msg == (int)Common.Win32.Msgs.WM_HOTKEY) && (m.WParam == (IntPtr)this.GetType().GetHashCode()))
            {
                Handled = true;
        ...
        {	//unregister hotkey
            return Common.Win32.User32.UnregisterHotKey(m_Window.Handle, this.GetType().GetHashCode());
        }
        ...
        {	//register hotkey
            int mod = 0;
            Keys k2 = Keys.None;
            if (((int)key & (int)Keys.Alt) == (int)Keys.Alt) { mod += (int)Common.Win32.Modifiers.MOD_ALT; k2 = Keys.Alt; }
            if (((int)key & (int)Keys.Shift) == (int)Keys.Shift) { mod += (int)Common.Win32.Modifiers.MOD_SHIFT; k2 = Keys.Shift; }
            if (((int)key & (int)Keys.Control) == (int)Keys.Control) { mod += (int)Common.Win32.Modifiers.MOD_CONTROL; k2 = Keys.Control; }

            System.Diagnostics.Debug.Write(mod.ToString() + " ");
            System.Diagnostics.Debug.WriteLine((((int)key) - ((int)k2)).ToString());

            return Common.Win32.User32.RegisterHotKey(m_Window.Handle, this.GetType().GetHashCode(), (int)mod, ((int)key) - ((int)k2));
        }
