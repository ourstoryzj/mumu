...
namespace DAL
{
    ...
    public class SignsService : ISignsService
    {
        ...
        public IList<Signs> search(int s, int e, string key, DateTime start, DateTime end, int uid, int state, int type, string orderby)
        {
            ...
            DBHelper.sqlstr = sql + sql1 + sql2 + sql3 + sql4 + sql5 + sql6 + " and id not in ( select top " + (s - 1).ToString() + " id from Signs where " + sql1 + sql2 + sql3 + sql4 + sql5 + sql6 + " ) " + sql7;
            List<Signs> list = new List<Signs>();
            SqlDataReader reader = DBHelper.ExecuteReader();
            while (reader.Read())
            {
        ...
        public static SqlParameter[] GetSqlParameters(Signs SignsExample)
        {
            List<SqlParameter> list_param = new List<SqlParameter>();
            if (SignsExample.SignTime != new DateTime() && SignsExample.SignTime != null)
            {
                list_param.Add(new SqlParameter("@SignTime", SignsExample.SignTime.ToString("yyyy-MM-dd")));
            }
            else
            {
                list_param.Add(new SqlParameter("@SignTime", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(SignsExample.Remark))
            {
                list_param.Add(new SqlParameter("@Remark", SignsExample.Remark));
            }
            else
            {
            ...
            {
                list_param.Add(new SqlParameter("@UId", SignsExample.UId));
            }
            else
            {
                list_param.Add(new SqlParameter("@UId", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(SignsExample.UName))
            {
                list_param.Add(new SqlParameter("@UName", SignsExample.UName));
            }
            else
            {
                list_param.Add(new SqlParameter("@UName", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(SignsExample.UState))
            {
                list_param.Add(new SqlParameter("@UState", SignsExample.UState));
            }
            else
            {
                list_param.Add(new SqlParameter("@UState", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(SignsExample.UType))
            {
                list_param.Add(new SqlParameter("@UType", SignsExample.UType));
            }
            else
            {
                list_param.Add(new SqlParameter("@UType", DBNull.Value));
            }
        ...
        public static Signs GetByReader(SqlDataReader Reader)
        {
