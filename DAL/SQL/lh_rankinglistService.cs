...
namespace DAL
{
    ...
    public class lh_rankinglistService : Ilh_rankinglistService
    {
        ...
        public static SqlParameter[] GetSqlParameters(lh_rankinglist lh_rankinglistExample)
        {
            List<SqlParameter> list_param = new List<SqlParameter>();

            if (!string.IsNullOrEmpty(lh_rankinglistExample.rkey))
            {
                list_param.Add(new SqlParameter("@rkey", lh_rankinglistExample.rkey));
            }
            else
            {
                list_param.Add(new SqlParameter("@rkey", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(lh_rankinglistExample.rstate))
            {
                list_param.Add(new SqlParameter("@rstate", lh_rankinglistExample.rstate));
            }
            else
            {
                list_param.Add(new SqlParameter("@rstate", DBNull.Value));
            }
            if (lh_rankinglistExample.rdate != new DateTime() && lh_rankinglistExample.rdate != null)
            {
                list_param.Add(new SqlParameter("@rdate", lh_rankinglistExample.rdate.ToString("yyyy-MM-dd")));
            }
            else
            {
                list_param.Add(new SqlParameter("@rdate", DBNull.Value));
            }
            if (lh_rankinglistExample.hid != 0)
            {
                list_param.Add(new SqlParameter("@hid", lh_rankinglistExample.hid));
            }
            else
            {
            ...
            {
                list_param.Add(new SqlParameter("@hremark", lh_rankinglistExample.hremark));
            }
            else
            {
        ...
        public static lh_rankinglist GetByReader(SqlDataReader Reader)
        {
