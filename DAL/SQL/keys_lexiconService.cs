...
namespace DAL
{
    ...
    public class keys_lexiconService : Ikeys_lexiconService
    {
        ...
        {
            string sql1 = "select top " + (e - s + 1).ToString() + " * from keys_lexicon where ";
            string sql2 = string.IsNullOrEmpty(key) ? " 1=1 " : " ( klname like '%" + key + "%' or klstate like '%" + key + "%' or klremark like '%" + key + "%' or klremark1 like '%" + key + "%' or klremark2 like '%" + key + "%' )";//删除无用字段，删除最后一个or
            string sql3 = string.IsNullOrEmpty(state) ? "" : " and klstate= '" + state + "' ";//状态字段，无用删除
            string sql5 = startdate == new DateTime() ? "" : " and DATEDIFF(D,'" + startdate.ToString("yyyy-MM-dd") + "',kldate)>=0 ";//时间字段，无用删除
            string sql6 = enddate == new DateTime() ? "" : " and DATEDIFF(D,'" + enddate.ToString("yyyy-MM-dd") + "',kldate)<=0 ";//时间字段，无用删除
        ...
        public static SqlParameter[] GetSqlParameters(keys_lexicon keys_lexiconExample)
        {
            List<SqlParameter> list_param = new List<SqlParameter>();

            if (!string.IsNullOrEmpty(keys_lexiconExample.klname))
            {
                list_param.Add(new SqlParameter("@klname", keys_lexiconExample.klname));
            }
            else
            {
                list_param.Add(new SqlParameter("@klname", DBNull.Value));
            }
            if (keys_lexiconExample.klsort != 0)
            {
                list_param.Add(new SqlParameter("@klsort", keys_lexiconExample.klsort));
            }
            else
            {
                list_param.Add(new SqlParameter("@klsort", DBNull.Value));
            }
            if (keys_lexiconExample.kldate != new DateTime() && keys_lexiconExample.kldate != null)
            {
                list_param.Add(new SqlParameter("@kldate", keys_lexiconExample.kldate.ToString("yyyy-MM-dd")));
            }
            else
            {
                list_param.Add(new SqlParameter("@kldate", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(keys_lexiconExample.klstate))
            {
                list_param.Add(new SqlParameter("@klstate", keys_lexiconExample.klstate));
            }
            else
            {
                list_param.Add(new SqlParameter("@klstate", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(keys_lexiconExample.klremark))
            {
                list_param.Add(new SqlParameter("@klremark", keys_lexiconExample.klremark));
            }
            else
            {
                list_param.Add(new SqlParameter("@klremark", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(keys_lexiconExample.klremark1))
            {
                list_param.Add(new SqlParameter("@klremark1", keys_lexiconExample.klremark1));
            }
            else
            {
                list_param.Add(new SqlParameter("@klremark1", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(keys_lexiconExample.klremark2))
            {
                list_param.Add(new SqlParameter("@klremark2", keys_lexiconExample.klremark2));
            }
            else
            {
                list_param.Add(new SqlParameter("@klremark2", DBNull.Value));
            }
        ...
        public static keys_lexicon GetByReader(SqlDataReader Reader)
        {
