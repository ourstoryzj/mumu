...
namespace DAL.Mysql
{
    ...
    public class subway_goodsService : Isubway_goodsService
    {
        ...
        {
            string sql1 = "select * from subway_goods where ";
            string sql2 = string.IsNullOrEmpty(key) ? " 1=1 " : " ( sgcampaignid like '%" + key + "%' or sgname like '%" + key + "%' or sgremark like '%" + key + "%' or sgstate like '%" + key + "%' or sgremark2 like '%" + key + "%'   )";//删除无用字段，删除最后一个or
            string sql3 = string.IsNullOrEmpty(state) ? "" : " and sgstate= '" + state + "' ";//状态字段，无用删除
            string sql4 = string.IsNullOrEmpty(sgcampaignid) ? "" : " and sgcampaignid= '" + sgcampaignid + "' ";//状态字段，无用删除
            string sql5 = startdate == new DateTime() ? "" : " and DATEDIFF(D,'" + startdate.ToString("yyyy-MM-dd") + "',sgdate)>=0 ";//时间字段，无用删除
        ...
        public static MySqlParameter[] GetSqlParameters(subway_goods subway_goodsExample)
        {
            List<MySqlParameter> list_param = new List<MySqlParameter>();

            if (!string.IsNullOrEmpty(subway_goodsExample.sgcampaignid))
            {
                list_param.Add(new MySqlParameter("@sgcampaignid", subway_goodsExample.sgcampaignid));
            }
            else
            {
                list_param.Add(new MySqlParameter("@sgcampaignid", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(subway_goodsExample.sgname))
            {
                list_param.Add(new MySqlParameter("@sgname", subway_goodsExample.sgname));
            }
            else
            {
                list_param.Add(new MySqlParameter("@sgname", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(subway_goodsExample.sgremark))
            {
                list_param.Add(new MySqlParameter("@sgremark", subway_goodsExample.sgremark));
            }
            else
            {
                list_param.Add(new MySqlParameter("@sgremark", DBNull.Value));
            }
            if (subway_goodsExample.sgdate != new DateTime() && subway_goodsExample.sgdate != null)
            {
                list_param.Add(new MySqlParameter("@sgdate", subway_goodsExample.sgdate.ToString("yyyy-MM-dd")));
            }
            else
            {
                list_param.Add(new MySqlParameter("@sgdate", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(subway_goodsExample.sgstate))
            {
                list_param.Add(new MySqlParameter("@sgstate", subway_goodsExample.sgstate));
            }
            else
            {
                list_param.Add(new MySqlParameter("@sgstate", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(subway_goodsExample.sgremark2))
            {
                list_param.Add(new MySqlParameter("@sgremark2", subway_goodsExample.sgremark2));
            }
            else
            {
                list_param.Add(new MySqlParameter("@sgremark2", DBNull.Value));
            }
        ...
        public static subway_goods GetByReader(MySqlDataReader Reader)
        {
