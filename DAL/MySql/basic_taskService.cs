...
namespace DAL.Mysql
{
    ...
    public class basic_taskService : Ibasic_taskService
    {
        ...
        {
            string sql1 = "select   *    from basic_task where ";
            string sql2 = string.IsNullOrEmpty(key) ? " 1=1 " : " (  btname like '%" + key + "%' or  btcontent     like '%" + key + "%' or  btspare1     like '%" + key + "%' or  btspare2     like '%" + key + "%'  or  btspare3     like '%" + key + "%'  or  btspare4     like '%" + key + "%'  or  btspare5     like '%" + key + "%'   ) ";
            string sql3 = string.IsNullOrEmpty(state) ? "" : " and btstate= '" + state + "' ";
            string sql4 = dt1_up == new DateTime() ? "" : " and datediff(day,'" + dt1_up.ToString("yyyy/MM/dd") + "',btdate)>=0 ";
            string sql5 = dt2_up == new DateTime() ? "" : " and datediff(day,'" + dt2_up.ToString("yyyy/MM/dd") + "',btdate)<=0 ";
            ...
            DBHelper.sqlstr = sql1 + sql2 + sql3 + sql4 + sql5 + sql6 + sql7  + sql12+ sql10;
            List<basic_task> list = new List<basic_task>();
            MySqlDataReader reader = DBHelper.ExecuteReader();
            while (reader.Read())
            {
        ...
        public IList<basic_task> Search(int s, int e, string key, string state, string chongfu, DateTime dt1_up, DateTime dt2_up, DateTime dt1_add, DateTime dt2_add, string orderby)
        {
            ...
            string sql12 = string.IsNullOrEmpty(orderby) ? " order by  btdate2 desc " : " order by " + orderby;
            string sql10 = e == 0 ? " " : " limit " + s + "," + e;
            DBHelper.sqlstr = sql1 + sql2 + sql3 + sql99 + sql4 + sql5 + sql6 + sql7  + sql12+ sql10;
            List<basic_task> list = new List<basic_task>();
            MySqlDataReader reader = DBHelper.ExecuteReader();
            while (reader.Read())
            {
        ...
        public static MySqlParameter[] GetSqlParameters(basic_task basic_taskExample)
        {
            List<MySqlParameter> list_param = new List<MySqlParameter>();

            if (!string.IsNullOrEmpty(basic_taskExample.btname))
            {
                list_param.Add(new MySqlParameter("@btname", basic_taskExample.btname));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btname", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_taskExample.btcontent))
            {
                list_param.Add(new MySqlParameter("@btcontent", basic_taskExample.btcontent));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btcontent", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_taskExample.btstate))
            {
                list_param.Add(new MySqlParameter("@btstate", basic_taskExample.btstate));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btstate", DBNull.Value));
            }
            if (basic_taskExample.btdate != new DateTime() && basic_taskExample.btdate != null)
            {
                list_param.Add(new MySqlParameter("@btdate", basic_taskExample.btdate.ToString("yyyy-MM-dd")));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btdate", DBNull.Value));
            }
            if (basic_taskExample.btdate2 != new DateTime() && basic_taskExample.btdate2 != null)
            {
                list_param.Add(new MySqlParameter("@btdate2", basic_taskExample.btdate2.ToString("yyyy-MM-dd")));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btdate2", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_taskExample.btspare1))
            {
                list_param.Add(new MySqlParameter("@btspare1", basic_taskExample.btspare1));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btspare1", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_taskExample.btspare2))
            {
                list_param.Add(new MySqlParameter("@btspare2", basic_taskExample.btspare2));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btspare2", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_taskExample.btspare3))
            {
                list_param.Add(new MySqlParameter("@btspare3", basic_taskExample.btspare3));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btspare3", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_taskExample.btspare4))
            {
                list_param.Add(new MySqlParameter("@btspare4", basic_taskExample.btspare4));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btspare4", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(basic_taskExample.btspare5))
            {
                list_param.Add(new MySqlParameter("@btspare5", basic_taskExample.btspare5));
            }
            else
            {
                list_param.Add(new MySqlParameter("@btspare5", DBNull.Value));
            }
        ...
        public static basic_task GetByReader(MySqlDataReader Reader)
        {
