...
namespace DAL.Mysql
{
    ...
    public class yh_keysService : Iyh_keysService
    {
        ...
        {
            DBHelper.sqlstr = "select * from yh_keys ";
        ...
        {
            DBHelper.sqlstr = "select * from yh_keys where kid = @kid";
        ...
        {
            DBHelper.sqlstr = "delete from yh_keys where kid =@kid";
            MySqlParameter[] param = new MySqlParameter[] {
                new MySqlParameter("@kid",kid)
			};
        ...
        public int SearchNum(string key, int ckid, decimal kdgbbsscs1, decimal kdgbbsscs2, decimal kdianjilv1, decimal kdianjilv2, int kdianjizhishu1, int kdianjizhishu2, int kgoodsnum1, int kgoodsnum2, decimal kqgbbcjcs1, decimal kqgbbcjcs2, int krenshu1, int krenshu2, decimal kscdianjizhanbi1, decimal kscdianjizhanbi2, decimal ksousuozhanbi1, decimal ksousuozhanbi2, decimal kzhitongche1, decimal kzhitongche2, decimal kzhuanhualv1, decimal kzhuanhualv2, int kcishu1, int kcishu2, string state, string state2, string state3, DateTime dt1_add, DateTime dt2_add)
        {
            ...
            DBHelper.sqlstr = sql1 + sql2 + sql3 + sql4 + sql5 + sql6 + sql7 + sql8 + sql9 + sql10 + sql11 + sql12 + sql13 + sql14 + sql15 + sql16 + sql17 + sql18 + sql19 + sql20 + sql21 + sql22 + sql23 + sql24 + sql25 + sql26 + sql27 + sql28;
        ...
        {
            string sql1 = "select  *   from yh_keys where ";
            string sql2 = string.IsNullOrEmpty(key) ? " 1=1 " : " (  kname like '%" + key + "%' or  kremark like '%" + key + "%'   ) ";
            string sql3 = string.IsNullOrEmpty(state) ? "" : " and kstate= '" + state + "' ";
            string sql4 = string.IsNullOrEmpty(state2) ? "" : " and kstate2= '" + state2 + "' ";
            string sql5 = string.IsNullOrEmpty(state3) ? "" : " and kstate3= '" + state3 + "' ";
            ...
            string sql28 = string.IsNullOrEmpty(orderby) ? " order by kdate desc " : " order by " + orderby;
            string sql100 = e == 0 ? " " : " limit " + s + "," + e;
            ...
            DBHelper.sqlstr = sql1 + sql2 + sql3 + sql4 + sql5 + sql6 + sql7 + sql8 + sql9 + sql10 + sql11 + sql12 + sql13 + sql14 + sql15 + sql16 + sql17 + sql18 + sql19 + sql20 + sql21 + sql22 + sql23 + sql24 + sql25 + sql26 + sql27 + sql30 + sql28+ sql100 ;
            List<yh_keys> list = new List<yh_keys>();
            MySqlDataReader reader = DBHelper.ExecuteReader();
            while (reader.Read())
            {
        ...
        public static MySqlParameter[] GetSqlParameter(yh_keys yh_keysExample)
        {
            List<MySqlParameter> list_param = new List<MySqlParameter>();
            if (yh_keysExample.ckid != 0)
            {
                list_param.Add(new MySqlParameter("@ckid", yh_keysExample.ckid));
            }
            else
            {
                list_param.Add(new MySqlParameter("@ckid", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_keysExample.k_standby1))
            {
                list_param.Add(new MySqlParameter("@k_standby1", yh_keysExample.k_standby1));
            }
            else
            {
                list_param.Add(new MySqlParameter("@k_standby1", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_keysExample.k_standby2))
            {
                list_param.Add(new MySqlParameter("@k_standby2", yh_keysExample.k_standby2));
            }
            else
            {
                list_param.Add(new MySqlParameter("@k_standby2", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_keysExample.k_standby3))
            {
                list_param.Add(new MySqlParameter("@k_standby3", yh_keysExample.k_standby3));
            }
            else
            {
                list_param.Add(new MySqlParameter("@k_standby3", DBNull.Value));
            }
            if (yh_keysExample.kcishu != 0)
            {
                list_param.Add(new MySqlParameter("@kcishu", yh_keysExample.kcishu));
            }
            else
            {
                list_param.Add(new MySqlParameter("@kcishu", DBNull.Value));
            }
            if (yh_keysExample.kdgbbsscs != new decimal())
            {
                list_param.Add(new MySqlParameter("@kdgbbsscs", yh_keysExample.kdgbbsscs));
            }
            else
            {
                list_param.Add(new MySqlParameter("@kdgbbsscs", DBNull.Value));
            }
            if (yh_keysExample.kdianjilv != new decimal())
            {
                list_param.Add(new MySqlParameter("@kdianjilv", yh_keysExample.kdianjilv));
            }
            else
            {
            ...
            {
                list_param.Add(new MySqlParameter("@kdianjizhishu", yh_keysExample.kdianjizhishu));
            }
            else
            {
                list_param.Add(new MySqlParameter("@kdianjizhishu", DBNull.Value));
            }
            if (yh_keysExample.kgoodsnum != 0)
            {
                list_param.Add(new MySqlParameter("@kgoodsnum", yh_keysExample.kgoodsnum));
            }
            else
            {
            ...
            {
                list_param.Add(new MySqlParameter("@kname", yh_keysExample.kname));
            }
            else
            {
            ...
            {
                list_param.Add(new MySqlParameter("@kqgbbcjcs", yh_keysExample.kqgbbcjcs));
            }
            else
            {
                list_param.Add(new MySqlParameter("@kqgbbcjcs", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_keysExample.kremark))
            {
                list_param.Add(new MySqlParameter("@kremark", yh_keysExample.kremark));
            }
            else
            {
            ...
            {
                list_param.Add(new MySqlParameter("@krenshu", yh_keysExample.krenshu));
            }
            else
            {
                list_param.Add(new MySqlParameter("@krenshu", DBNull.Value));
            }
            if (yh_keysExample.kscdianjizhanbi != new decimal())
            {
                list_param.Add(new MySqlParameter("@kscdianjizhanbi", yh_keysExample.kscdianjizhanbi));
            }
            else
            {
                list_param.Add(new MySqlParameter("@kscdianjizhanbi", DBNull.Value));
            }
            if (yh_keysExample.ksousuozhanbi != new decimal())
            {
                list_param.Add(new MySqlParameter("@ksousuozhanbi", yh_keysExample.ksousuozhanbi));
            }
            else
            {
                list_param.Add(new MySqlParameter("@ksousuozhanbi", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(yh_keysExample.kstate))
            {
                list_param.Add(new MySqlParameter("@kstate", yh_keysExample.kstate));
            }
            else
            {
            ...
            {
                list_param.Add(new MySqlParameter("@kzhitongche", yh_keysExample.kzhitongche));
            }
            else
            {
                list_param.Add(new MySqlParameter("@kzhitongche", DBNull.Value));
            }
            if (yh_keysExample.kzhuanhualv != new decimal())
            {
                list_param.Add(new MySqlParameter("@kzhuanhualv", yh_keysExample.kzhuanhualv));
            }
            else
            {
        ...
        public static yh_keys GetByReader(MySqlDataReader Reader)
        {
