...
namespace DAL.MySql
{
    ...
    public class wanggong_dianpuService : Iwanggong_dianpuService
    {
        ...
        {
            string sql1 = "select  *   from wanggong_dianpu where ";
            string sql2 = string.IsNullOrEmpty(key) ? " 1=1 " : " ( wdpname  like '%" + key + "%' or  wdTBID like '%" + key + "%' or  wdwangwang like '%" + key + "%'  or  wdremark1  like '%" + key + "%' or  wdremark2  like '%" + key + "%'or  wdremark3  like '%" + key + "%'or  wdremark4  like '%" + key + "%' ) ";
            DBHelper.sqlstr = sql1 + sql2;
            string sql3 = string.IsNullOrEmpty(state) ? " " : " and wdstate='" + state + "' ";
            string sql4 = begin == new DateTime() ? "" : " and datediff(d,'" + begin.ToString("yyyy-MM-dd") + "',wdcollectdate)>=0 ";
            ...
            DBHelper.sqlstr = sql1 + sql2 + sql4 + sql3+sql10;
            List<wanggong_dianpu> list = new List<wanggong_dianpu>();
            MySqlDataReader reader = DBHelper.ExecuteReader();
            while (reader.Read())
            {
        ...
        public static MySqlParameter[] GetSqlParameters(wanggong_dianpu wanggong_dianpuExample)
        {
            List<MySqlParameter> list_param = new List<MySqlParameter>();

            if (!string.IsNullOrEmpty(wanggong_dianpuExample.wdpname))
            {
                list_param.Add(new MySqlParameter("@wdpname", wanggong_dianpuExample.wdpname));
            }
            else
            {
                list_param.Add(new MySqlParameter("@wdpname", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(wanggong_dianpuExample.wdTBID))
            {
                list_param.Add(new MySqlParameter("@wdTBID", wanggong_dianpuExample.wdTBID));
            }
            else
            {
                list_param.Add(new MySqlParameter("@wdTBID", DBNull.Value));
            }
            if (wanggong_dianpuExample.wdcollectdate != new DateTime() && wanggong_dianpuExample.wdcollectdate != null)
            {
                list_param.Add(new MySqlParameter("@wdcollectdate", wanggong_dianpuExample.wdcollectdate.ToString("yyyy-MM-dd")));
            }
            else
            {
                list_param.Add(new MySqlParameter("@wdcollectdate", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(wanggong_dianpuExample.wdwangwang))
            {
                list_param.Add(new MySqlParameter("@wdwangwang", wanggong_dianpuExample.wdwangwang));
            }
            else
            {
                list_param.Add(new MySqlParameter("@wdwangwang", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(wanggong_dianpuExample.wdstate))
            {
                list_param.Add(new MySqlParameter("@wdstate", wanggong_dianpuExample.wdstate));
            }
            else
            {
                list_param.Add(new MySqlParameter("@wdstate", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(wanggong_dianpuExample.wdremark1))
            {
                list_param.Add(new MySqlParameter("@wdremark1", wanggong_dianpuExample.wdremark1));
            }
            else
            {
                list_param.Add(new MySqlParameter("@wdremark1", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(wanggong_dianpuExample.wdremark2))
            {
                list_param.Add(new MySqlParameter("@wdremark2", wanggong_dianpuExample.wdremark2));
            }
            else
            {
                list_param.Add(new MySqlParameter("@wdremark2", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(wanggong_dianpuExample.wdremark3))
            {
                list_param.Add(new MySqlParameter("@wdremark3", wanggong_dianpuExample.wdremark3));
            }
            else
            {
                list_param.Add(new MySqlParameter("@wdremark3", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(wanggong_dianpuExample.wdremark4))
            {
                list_param.Add(new MySqlParameter("@wdremark4", wanggong_dianpuExample.wdremark4));
            }
            else
            {
                list_param.Add(new MySqlParameter("@wdremark4", DBNull.Value));
            }
        ...
        public static wanggong_dianpu GetByReader(MySqlDataReader Reader)
        {
