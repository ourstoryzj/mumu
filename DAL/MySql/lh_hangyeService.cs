...
namespace DAL.MySql
{
    ...
    public class lh_hangyeService : Ilh_hangyeService
    {
        ...
        {
            string sql1 = "select * from lh_hangye where ";
            string sql2 = string.IsNullOrEmpty(key) ? " 1=1 " : " ( hname like '%" + key + "%' or hstate like '%" + key + "%' or hremark like '%" + key + "%'  )";//删除无用字段，删除最后一个or
            string sql3 = string.IsNullOrEmpty(state) ? "" : " and hstate= '" + state + "' ";//状态字段，无用删除
            //string sql4 = id == 0 ? "" : " and id字段='" + id.ToString() + "' ";//Int字段，无用删除
            string sql5 = startdate == new DateTime() ? "" : " and DATEDIFF(D,'" + startdate.ToString("yyyy-MM-dd") + "',hdate)>=0 ";//时间字段，无用删除
        ...
        public static MySqlParameter[] GetSqlParameters(lh_hangye lh_hangyeExample)
        {
            List<MySqlParameter> list_param = new List<MySqlParameter>();

            if (!string.IsNullOrEmpty(lh_hangyeExample.hname))
            {
                list_param.Add(new MySqlParameter("@hname", lh_hangyeExample.hname));
            }
            else
            {
                list_param.Add(new MySqlParameter("@hname", DBNull.Value));
            }

            if (!string.IsNullOrEmpty(lh_hangyeExample.hstate))
            {
                list_param.Add(new MySqlParameter("@hstate", lh_hangyeExample.hstate));
            }
            else
            {
            ...
            {
                list_param.Add(new MySqlParameter("@hdate", lh_hangyeExample.hdate.ToString("yyyy-MM-dd")));
            }
            else
            {
            ...
            {
                list_param.Add(new MySqlParameter("@hremark", lh_hangyeExample.hremark));
            }
            else
            {
        ...
        public static lh_hangye GetByReader(MySqlDataReader Reader)
        {
